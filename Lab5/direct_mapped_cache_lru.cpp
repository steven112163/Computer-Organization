//-------------------------------------------
// Subject: Lab5 - direct_mapped_cache_lru
//-------------------------------------------
...
void simulate(char* argv[]) {
	...
	for (int i = 0; i < m; i++) {
		...
		for (int j = 0; j < p; j++) {
			...
			// beq $6, $0, end_k
			for (int k = 0; k < n; k++) {
				unsigned int temp1 = 4 * (i*p + j) + addressC;
				programExecutionCycle += 5;
				// mul $7, $3, $23
				// addu $8, $7, $4
				// mul $8, $8, $1
				...
				getDataAB(cache, (temp1 >> offset) & (set - 1),
					temp1 >> (index + offset),
				...
				getDataC(cacheL1, cacheL2,
					(temp1 >> offsetL1) & (setL1 - 1),
					temp1 >> (indexL1 + offsetL1),
					(temp1 >> offsetL2) & (setL2 - 1),
					temp1 >> (indexL2 + offsetL2), stallCycle1C);
				getDataC(cacheL1, cacheL2,
